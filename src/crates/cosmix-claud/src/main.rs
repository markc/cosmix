//! cosmix-claud — Claude AI Bus port daemon with knowledge-augmented generation.
//!
//! Listens on `/run/user/{uid}/cosmix/ports/claud.sock` and exposes
//! LLM operations as Bus commands. Automatically enriches prompts with
...
//!   mix.execute  — Run a Mix script and return stdout output
//!   mix.expr     — Evaluate a Mix expression and return the result value
...
async fn main() {
    let _log = cosmix_log::init(
        ...
        cosmix_log::LogDefaults::daemon("cosmix-claud").with_stats(false),
    )
...
async fn handle_request(stream: &mut tokio::net::UnixStream, state: &State) -> anyhow::Result<()> {
    ...
    let (rc, body) = match command.as_str() {
        ...
        "mix.execute" => handle_mix_execute(&args).await,
        "mix.expr" => handle_mix_expr(&args).await,
        "help" => (
            0,
            serde_json::json!({
        ...
        "info" => (
            ...
            serde_json::json!({
                ...
                "domain": &state.domain,
                "commands": 8,
                "command_list": ["ask", "ask_raw", "analyze", "generate", "mix.execute", "mix.expr", "help", "info"],
            }),
        ),
        _ => (
            10,
...
async fn handle_ask(
        ...
        .or_else(|| args.as_str())
    {
        ...
        None => {
            return (
                10,
                serde_json::json!({"error": "Missing 'prompt' argument"}),
            );
        }
    };
    ...
    match llm.complete(&system, &prompt).await {
        ...
        Err(e) => (
            10,
            serde_json::json!({"error": format!("LLM call failed: {e}")}),
        ),
    }
}
...
async fn handle_analyze(args: &serde_json::Value, state: &State) -> (u8, serde_json::Value) {
    ...
    if code.is_empty() && error_msg.is_empty() {
        return (
            10,
            serde_json::json!({"error": "Provide 'code' and/or 'error' arguments"}),
        );
    }
    ...
    let (rc, mut body) = handle_ask(&serde_json::json!({"prompt": prompt}), state, true).await;
...
// ---- Mix script execution ----

/// Execute a Mix script and return stdout output.
/// Runs on a blocking thread since the Mix evaluator is !Send.
async fn handle_mix_execute(args: &serde_json::Value) -> (u8, serde_json::Value) {
        ...
        .or_else(|| args.as_str())
    {
        ...
        None => {
            return (
                10,
                serde_json::json!({"error": "Missing 'script' argument"}),
            );
        }
    };

    // Collect variables to inject (must be Send for spawn_blocking)
    let vars: Vec<(String, serde_json::Value)> = args
        .get("vars")
    ...
    match tokio::task::spawn_blocking(move || {
        let rt = tokio::runtime::Builder::new_current_thread()
            ...
            .build()
            .unwrap();
        rt.block_on(async move { run_mix_script_claud(&script, &vars).await })
    })
    .await
    {
        Ok(result) => result,
        Err(e) => (
            10,
            serde_json::json!({"error": format!("task panicked: {e}")}),
        ),
    }
}

/// Evaluate a Mix expression and return the result value as JSON.
/// Runs on a blocking thread since the Mix evaluator is !Send.
async fn handle_mix_expr(args: &serde_json::Value) -> (u8, serde_json::Value) {
    let expr = match args
        .get("expr")
        .and_then(|v| v.as_str())
        .or_else(|| args.as_str())
    {
        Some(s) => s.to_string(),
        None => return (10, serde_json::json!({"error": "Missing 'expr' argument"})),
    };
    ...
    match tokio::task::spawn_blocking(move || {
        ...
        rt.block_on(async move {
            // Wrap expression in a return statement to capture the value
            ...
            match eval.execute(&stmts).await {
                // Fallible since cosmix-lib-mix 0.21.0: a non-finite number
                // in the result has no JSON representation → rc=10 error,
                // never a silently-mangled 0.
                Ok(val) => match cosmix_mix::json::mix_to_json(&val) {
                    Ok(json_val) => (0, serde_json::json!({"result": json_val})),
                    Err(e) => (
                        10,
                        serde_json::json!({"error": format!("result not JSON-encodable: {e}")}),
                    ),
                },
                Err(e) => (10, serde_json::json!({"error": e.to_string()})),
            }
        })
    })
    .await
...
/// Inner Mix execution logic (called from spawn_blocking thread).
async fn run_mix_script_claud(
    script: &str,
    ...
    // Inject variables as Mix globals
    for (k, v) in vars {
        let val = match v {
            serde_json::Value::String(s) => cosmix_mix::value::Value::String(s.clone()),
            serde_json::Value::Number(n) => {
                cosmix_mix::value::Value::Number(n.as_f64().unwrap_or(0.0))
            }
            serde_json::Value::Bool(b) => cosmix_mix::value::Value::Bool(*b),
            _ => cosmix_mix::value::Value::String(v.to_string()),
        };
        eval.set_global(k, val);
    }
    ...
    match eval.execute(&stmts).await {
        Ok(_) => {
            ...
            let mut result = serde_json::json!({"output": out});
            if !err.is_empty() {
                result["stderr"] = serde_json::json!(err);
            }
            (0, result)
        }
        Err(e) => {
            let out = stdout.to_string_lossy();
            let mut result = serde_json::json!({"error": e.to_string()});
            if !out.is_empty() {
                result["partial_output"] = serde_json::json!(out);
            }
            (10, result)
        }
    }
...
async fn maybe_extract_skill(
    ...
    let transcript = cosmix_skills::TaskTranscript {
        ...
        messages: vec![
            cosmix_skills::Message {
                role: "user".into(),
                content: truncate(prompt, 1000),
            },
            cosmix_skills::Message {
                role: "assistant".into(),
                content: truncate(response, 1000),
            },
        ],
        tool_calls: vec![],
