...
pub async fn evaluate_task(
    ...
    let tools_used: Vec<&str> = transcript
        .tool_calls
        .iter()
        .map(|t| t.name.as_str())
        .collect();
...
pub async fn extract_skill(
    ...
    let raw: ExtractedSkillRaw =
        parse_json_response(&response).context("parsing extracted skill")?;
...
pub async fn retrieve_skills_domain(
    ...
    let results = indexd
        .search_skills_domain(task_description, max_skills, domain)
        .await?;
    Ok(results
        .into_iter()
...
pub async fn refine_skill(
    ...
    let updated = SkillDocument {
        ...
        source_commit: existing.source_commit.clone(),
...
pub async fn check_graduation(
    ...
    let skills_cfg = cosmix_config::store::load_service::<cosmix_config::SkillsSettings>("skills")
        .unwrap_or_default();
    let threshold_conf = skills_cfg.graduation_confidence as f32;
    let threshold_uses = skills_cfg.graduation_min_uses;
    let threshold_successes = skills_cfg.graduation_min_successes;
    ...
    //
    // DESIGN PRINCIPLE: Human-authored vs LLM-derived separation.
    // Graduation ONLY appends to the dedicated auto-generated section at the
    // end of the file. It NEVER modifies, rewrites, or reorders human-authored
    // content above the marker. If a graduated skill contradicts a human-authored
    // section, the human-authored section wins — skills are derived, docs are
    // authoritative.
    let content = std::fs::read_to_string(&claude_md_path)
        .with_context(|| format!("reading {}", claude_md_path.display()))?;

    let section_marker = "## Graduated Skills (auto-generated)";
    let updated = if let Some(marker_pos) = content.find(section_marker) {
        // Append at the END of the file, after the marker section.
        // This ensures we never insert into or reorder human-authored content.
        let marker_end = marker_pos + section_marker.len();
        format!(
            "{}{}\n\n{rule}",
            &content[..marker_end],
            &content[marker_end..]
        ...
        // Create section at end of file — never in the middle
        format!("{content}\n\n{section_marker}\n\n{rule}")
    };
...
fn find_claude_md_for_domain(domain: &str) -> Result<std::path::PathBuf> {
    ...
    // Try reverse lookup in domains.toml `map` first
    if let Ok(domains) =
        cosmix_config::store::load_service::<cosmix_config::DomainsSettings>("domains")
    {
        for (prefix, d) in &domains.map {
            if d == domain {
                ...
                let claude_md = std::path::PathBuf::from(&expanded).join("CLAUDE.md");
                if claude_md.exists() {
                    return Ok(claude_md);
                }
            }
    ...
    // Fallback: domain segments → dot-prefixed path
    ...
    if claude_md.exists() {
        ...
        anyhow::bail!(
            "CLAUDE.md not found for domain '{}' at {}",
            domain,
            claude_md.display()
        )
    }
