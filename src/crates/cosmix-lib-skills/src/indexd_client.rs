...
impl IndexdClient {
    /// Connect using the socket path from `~/.config/cosmix/indexd.toml [service]`.
    pub async fn from_config() -> Result<Self> {
        let cfg = cosmix_config::store::load_service::<cosmix_config::IndexdSettings>("indexd")
            .unwrap_or_default();
        Self::connect(Some(&cfg.service.socket_path)).await
    }
    ...
    pub async fn search_skills(
        ...
        self.search_skills_domain(task_description, limit, None)
            .await
    }
