...
/// Unique identifier for a track within a song.
...
impl TrackId {
    ...
    /// Advances the global counter past `id`. The counter is process-global
    /// and restarts at 1, so every deserialization path must reseed it or
    /// freshly allocated IDs could collide with loaded ones.
    pub(crate) fn bump_counter_past(id: u64) {
        TRACK_ID_COUNTER.fetch_max(id.saturating_add(1), Ordering::Relaxed);
    }
}
...
impl Track {
    /// Creates a new track with default settings: default instrument
    /// (piano), centered pan, and full volume.
    ...
    /// Creates a drum track on channel 9 (the General MIDI drum channel).
    ...
    /// Removes a note by its ID. Returns the removed note, or None if not found.
    ...
    /// Returns notes that overlap the tick range [start, end).
    pub fn notes_in_range(&self, start: u32, end: u32) -> impl Iterator<Item = &Note> {
        // A note starting long before `start` can still overlap the range, so
        // there is no sound lower bound to binary-search for without tracking
        // a maximum duration. Scan from the front, bounded by the sorted-order
        // take_while — fine at document-model scale.
        self.notes
            .iter()
    ...
    /// Quantizes all note start positions to a grid of `grid_ticks`.
    ...
    /// Returns the number of notes that couldn't be transposed (out of range).
...
mod tests {
    ...
    #[test]
    fn test_notes_in_range_long_early_note() {
        let mut track = Track::new("Test", 0);
        track.create_note(60, 100, 0, 2000); // 0-2000, spans the whole range
        track.create_note(62, 100, 960, 240); // 960-1200

        let in_range: Vec<_> = track.notes_in_range(1500, 1600).collect();
