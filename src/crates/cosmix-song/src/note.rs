...
/// Unique identifier for a note within a song.
...
impl NoteId {
    ...
    pub(crate) fn bump_counter_past(id: u64) {
        NOTE_ID_COUNTER.fetch_max(id.saturating_add(1), Ordering::Relaxed);
    }
}
...
impl Note {
    /// Creates a new note with a unique ID. Pitch and velocity are clamped
    /// to the MIDI 0-127 range.
    ///
    /// # Examples
    ///
    /// ```
    /// use cosmix_song::Note;
    ///
    ...
    /// Checks if any part of the note falls within [start, end).
    ...
    /// Transposes the note by a number of semitones (can be negative).
    ///
    /// Returns true if the transposition was successful (note stays in the
    /// 0-127 range); on failure the note is unchanged.
    ...
    /// Moves the note by a number of ticks (can be negative), saturating at 0.
