...
pub async fn set(db: &Db, account_id: i32, args: serde_json::Value) -> Result<serde_json::Value> {
    ...
    if let Some(create) = args.get("create").and_then(|v| v.as_object()) {
        for (client_id, obj) in create {
            ...
            match db::calendar::create_calendar(
                &db.conn,
                account_id,
                name,
                color,
...
pub async fn changes(
    ...
    let max = args
        ...
        .unwrap_or(500);
    let result =
        db::changelog::changes_since(&db.conn, account_id, "Calendar", since_state, max).await?;
...
pub async fn event_query(
    ...
    let (ids, total) = db::calendar::query_event_ids(
        &db.conn,
        account_id,
        calendar_id,
        after,
...
pub async fn event_set(
    ...
    if let Some(create) = args.get("create").and_then(|v| v.as_object()) {
        for (client_id, obj) in create {
            ...
            match db::calendar::create_event(
                &db.conn,
                account_id,
                calendar_id,
                &uid,
                obj,
                db::calendar::EventMetadata {
                    title,
                    start_dt,
                    end_dt,
                },
            ...
            {
                Ok(id) => {
                    db::changelog::record(&db.conn, account_id, "CalendarEvent", id, "created")
                        .await?;
                    created_map.insert(
                        client_id.clone(),
    ...
    if let Some(update) = args.get("update").and_then(|v| v.as_object()) {
        for (id_str, patch) in update {
            ...
            let patch_title = patch.get("title").and_then(|v| v.as_str());
            let patch_start = patch
                .get("start")
                .and_then(|v| v.as_str())
                .and_then(parse_rfc);
            let patch_end = patch
            ...
            // MERGE, don't replace (see Contact/set update): keep unspecified
            // indexed columns AND the verbatim iCal (RRULE/VALARM/X-props).
            let existing = db::calendar::get_events_by_ids(&db.conn, account_id, &[id]).await?;
            let Some(cur) = existing.first() else {
                continue; // no such event → nothing to update
            };
            let title = patch_title.or(cur.title.as_deref());
            let start_dt = patch_start.or_else(|| cur.start_dt.as_deref().and_then(parse_rfc));
            let end_dt = patch_end.or_else(|| cur.end_dt.as_deref().and_then(parse_rfc));

            let mut data = cur.data.clone();
            if let Some(t) = patch_title {
                data["title"] = serde_json::Value::String(t.to_string());
            }
            ...
            {
                let patched =
                    cosmix_lib_davproto::ical::patch_ics(raw, patch_title, patch_start, patch_end);
                data[cosmix_lib_davproto::ical::RAW_ICAL_KEY] = serde_json::Value::String(patched);
            }
            if db::calendar::update_event(&db.conn, account_id, id, &data, title, start_dt, end_dt)
                .await?
            {
    ...
    if let Some(destroy) = args.get("destroy").and_then(|v| v.as_array()) {
        for id_val in destroy {
            ...
            if db::calendar::delete_event(&db.conn, account_id, id).await? {
                db::changelog::record(&db.conn, account_id, "CalendarEvent", id, "destroyed")
                    .await?;
                destroyed_list.push(id_str.to_string());
            }
        }
...
pub async fn event_changes(
    ...
    let max = args
        ...
        .unwrap_or(500);
    let result =
        db::changelog::changes_since(&db.conn, account_id, "CalendarEvent", since_state, max)
            .await?;
    let resp = ChangesResponse {
        account_id: acct,
        old_state: since_state.to_string(),
