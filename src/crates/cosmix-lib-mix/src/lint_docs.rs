...
pub const LINT_DOCS: &[LintDoc] = &[
    ...
    LintDoc {
        ...
        detail: "A file run as a script has no well-formed `-- version: X.Y.Z` header in its first 32 lines, so `mix SCRIPT --version` and `script_version()` can only report it as `unversioned` — Mark's fleet rule (2026-09-25) is that every binary AND every Mix script answers `--version` with build details. \"Run as a script\" is a deliberately simple heuristic: line 1 is a `#!` shebang, or the path AS GIVEN has a `bin` or `_bin` directory component; a `lib/` module loaded by `require` is neither and is never reported. A header whose value is not `X.Y.Z` (optionally `-pre` / `+build`) is reported on its own line, because the runtime treats it as absent and a typo would otherwise pass silently. Only one spelling counts: `-- version: X.Y.Z` (whitespace-tolerant, lowercase, colon required). Severity `note` by default; `mix lint --require-version` reports it as a `warning` with the code unchanged, so `--require-version --deny-warnings` makes a missing header fail a gate. Emitted by the lint driver, not the analyzer, because it is a property of the file, not the program.",
    },
    LintDoc {
        code: "MIX-D3014",
