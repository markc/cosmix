...
use ash::vk;
// Cosmix downstream: opt-in, clock-free block lifetime observations.
pub mod diagnostics;
use log::{debug, Level};
...
impl MemoryBlock {
    fn new(
        ...
        diagnostics::record("comp_vk_block_alloc", || {
            use ash::vk::Handle;
            (device_memory.as_raw(), size, mem_type_index as u64)
        });
        ...
        let mapped_ptr = mapped
            .then(|| {
                ...
                .map_err(|e| {
                    ...
                    diagnostics::record("comp_vk_block_free", || {
                        use ash::vk::Handle;
                        (device_memory.as_raw(), size, 0)
                    });
    ...
    fn destroy(self, device: &ash::Device) {
        ...
        diagnostics::record("comp_vk_block_free", || {
            use ash::vk::Handle;
            (self.device_memory.as_raw(), self.size, 0)
        });
    }
}
