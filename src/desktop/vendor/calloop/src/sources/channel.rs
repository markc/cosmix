...
impl<T> EventSource for Channel<T> {
        ...
        C: FnMut(Self::Event, &mut Self::Metadata) -> Self::Ret,
    {
        ...
        } else if !matched || clear_readiness {
            Ok(action)
        } else {
            // Re-notify only after servicing our own token and exhausting the
            // batch limit. An unrelated sub-source token did not inspect this
            // receiver, so re-pinging it would create a perpetual wakeup loop.
            self.ping.ping();
...
mod tests {
    ...
    #[test]
    fn composed_channels_do_not_ping_each_other_for_unrelated_tokens() {
        struct Pair {
            first: Channel<()>,
            second: Channel<()>,
            dispatches: std::rc::Rc<std::cell::Cell<usize>>,
        }
        impl EventSource for Pair {
            type Event = Event<()>;
            type Metadata = ();
            type Ret = ();
            type Error = ChannelError;
            fn process_events<C>(&mut self, readiness: Readiness, token: Token, mut callback: C)
                -> Result<PostAction, Self::Error>
            where C: FnMut(Self::Event, &mut ()) {
                self.dispatches.set(self.dispatches.get() + 1);
                self.first.process_events(readiness, token, &mut callback)?;
                self.second.process_events(readiness, token, callback)?;
                Ok(PostAction::Continue)
            }
            fn register(&mut self, poll: &mut Poll, factory: &mut TokenFactory) -> crate::Result<()> {
                self.first.register(poll, factory)?;
                self.second.register(poll, factory)
            }
            fn reregister(&mut self, poll: &mut Poll, factory: &mut TokenFactory) -> crate::Result<()> {
                self.first.reregister(poll, factory)?;
                self.second.reregister(poll, factory)
            }
            fn unregister(&mut self, poll: &mut Poll) -> crate::Result<()> {
                self.first.unregister(poll)?;
                self.second.unregister(poll)
            }
        }
        let (first_tx, first) = channel();
        let (_second_tx, second) = channel();
        let dispatches = std::rc::Rc::new(std::cell::Cell::new(0));
        let mut event_loop = crate::EventLoop::try_new().unwrap();
        event_loop.handle().insert_source(Pair {
            first, second, dispatches: dispatches.clone(),
        }, |event, _, messages: &mut usize| {
            if matches!(event, Event::Msg(())) { *messages += 1; }
        }).unwrap();
        first_tx.send(()).unwrap();
        let mut messages = 0;
        for _ in 0..8 {
            event_loop.dispatch(Some(std::time::Duration::ZERO), &mut messages).unwrap();
        }
        assert_eq!(messages, 1);
        assert_eq!(dispatches.get(), 1, "empty channels must not manufacture wakeups");
    }
