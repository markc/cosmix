...
impl Device {
    ...
    /// Creates a [`Texture`] from a wgpu-hal texture in a known initial usage.
    ///
    /// Unlike [`Self::create_texture_from_hal`], this seeds wgpu-core's usage
    /// tracker with `initial_usage` instead of `UNINITIALIZED`. The returned
    /// usage is the seed installed by wgpu-core.
    ///
    /// # Safety
    ///
    /// - `hal_texture` must be created from this device internal handle
    /// - `hal_texture` must be created respecting `desc`
    /// - `hal_texture` must be initialised and currently in `initial_usage`
    #[cfg(wgpu_core)]
    #[must_use]
    pub unsafe fn create_texture_from_hal_with_initial_usage<A: hal::Api>(
        &self,
        hal_texture: A::Texture,
        desc: &TextureDescriptor<'_>,
        initial_usage: wgt::TextureUses,
    ) -> (Texture, wgt::TextureUses) {
        let (texture, tracker_seed) = unsafe {
            let core_device = self.inner.as_core();
            core_device
                .context
                .create_texture_from_hal_with_initial_usage::<A>(
                    hal_texture,
                    core_device,
                    desc,
                    initial_usage,
