...
impl ContextWgpuCore {
    ...
    pub unsafe fn create_texture_from_hal<A: hal::Api>(
        ...
        let (texture, _) = unsafe {
            self.create_texture_from_hal_with_initial_usage::<A>(
                hal_texture,
                device,
                desc,
    ...
    ) -> (CoreTexture, wgt::TextureUses) {
        let descriptor = desc.map_label_and_view_formats(|l| l.map(Borrowed), |v| v.to_vec());
        let (id, error, tracker_seed) = unsafe {
            self.0.create_texture_from_hal_with_initial_usage(
                Box::new(hal_texture),
                device.id,
                &descriptor,
                initial_usage,
