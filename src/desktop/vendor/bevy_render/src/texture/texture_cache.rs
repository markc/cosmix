use crate::{
    render_resource::{Texture, TextureId, TextureView},
...
pub struct TextureCache {
    ...
    // Cosmix: separate frame ageing from releasing per-update reservations.
    retained: HashSet<TextureId>,
    capture_usage: bool,
    recently_used: Vec<TextureId>,
}

impl TextureCache {
    /// Cosmix host policy: retention applies to the next cleanup only; usage
    /// capture stays enabled once requested. Entries remain available to all
    /// preparation systems; reservations are always released by update.
    pub fn set_update_policy(&mut self, retained: impl IntoIterator<Item = TextureId>) {
        self.retained.clear();
        self.retained.extend(retained);
        self.capture_usage = true;
    }

    /// Textures actually requested during the most recently cleaned-up update.
    /// Populated only after a host opts in with set_update_policy.
    pub fn recently_used(&self) -> &[TextureId] {
        &self.recently_used
    }
    ...
    pub fn update(&mut self) {
        ...
        self.textures.retain(|_, textures| {
            for texture in textures.iter_mut() {
                if self.capture_usage && texture.taken {
                    self.recently_used.push(texture.texture.id());
                }
                if !self.retained.contains(&texture.texture.id()) {
