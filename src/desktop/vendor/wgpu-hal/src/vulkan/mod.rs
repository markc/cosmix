...
impl crate::Queue for Queue {
    ...
    unsafe fn submit(
        ...
        // Cosmix downstream: timing only; preserve semaphore, lock and submit ordering.
        let diagnostic = crate::diagnostics::begin(crate::diagnostics::Operation::Bookkeeping);
        ...
        drop(diagnostic);
        unsafe {
            let diagnostic = crate::diagnostics::begin(crate::diagnostics::Operation::VulkanSubmit);
            let result = self
                .device
                .raw
                .queue_submit(self.raw, &[vk_info], fence_raw);
            drop(diagnostic);
            result.map_err(map_host_device_oom_and_lost_err)?
        };
        Ok(())
    }
