...
impl Global {
    ...
    pub unsafe fn create_texture_from_hal(
        ...
        let (id, error, _) = unsafe {
            self.create_texture_from_hal_with_initial_usage(
                hal_texture,
                device_id,
                desc,
    ...
    /// Creates a texture from a HAL texture whose current usage is known.
    ///
    /// # Safety
    ///
    /// In addition to the requirements of [`Self::create_texture_from_hal`],
    /// `initial_usage` must describe the HAL texture's actual current state.
    pub unsafe fn create_texture_from_hal_with_initial_usage(
        &self,
        hal_texture: Box<dyn hal::DynTexture>,
        device_id: DeviceId,
        desc: &resource::TextureDescriptor,
        ...
        let error = 'error: {
            ...
            let texture = match device.create_texture_from_hal(hal_texture, desc, initial_usage) {
                Ok(texture) => texture,
                Err(error) => break 'error error,
            };
            ...
            return (id, None, initial_usage);
        };

        let id = fid.assign(Fallible::Invalid(Arc::new(desc.label.to_string())));
        (id, Some(error), initial_usage)
    }
