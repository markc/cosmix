...
pub fn create_pty_with_spawn(
    ...
    create_pty_with_spawn_fd(shell, args, working_directory, env, columns, rows, width, height, None)
}

/// Cosmix's additive launch hook. Both fds must be distinct, open, reserved
/// CLOEXEC descriptors owned by the caller until this function returns. Only
/// the forked child duplicates source onto target; the parent flags never change.
#[allow(clippy::too_many_arguments)]
pub fn create_pty_with_spawn_fd(
    shell: Option<&str>,
    args: Vec<String>,
    working_directory: &Option<String>,
    env: Option<Vec<(String, String)>>,
    ...
    width: u16,
    height: u16,
    inherit_fd: Option<(RawFd, RawFd)>,
) -> Result<Pty, Error> {
    if inherit_fd.is_some_and(|(source, target)| source < 3 || target < 3 || source == target) {
        return Err(Error::new(std::io::ErrorKind::InvalidInput, "invalid inherited fd mapping"));
    }
