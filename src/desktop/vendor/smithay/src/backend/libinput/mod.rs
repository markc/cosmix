...
// COSMIX PATCH: touch slots must be seat-wide, not device-local.
//
// Upstream forwards libinput's `slot()`, which is the *device's* MT slot — or
// `None` for a single-touch device, which Smithay maps to the `wl_touch` id
// `-1`. `TouchInternal` keys its focus map on that slot for the whole seat, and
// the id goes straight onto the wire, so two touch devices on one seat collide:
// two single-touch panels both report `None`, two multitouch panels both start
// at hardware slot 0. The second down overwrites the first's focus entry and
// re-uses an id the client still has live, after which the first device's motion
// and up are misattributed or dropped.
//
// libinput already solves this — `seat_slot()` is allocated across the seat and
// is unique among simultaneously active contacts, which is exactly the
// uniqueness `wl_touch` ids require. It is a plain `u32` rather than an
// `Option`, because a seat slot is always assigned. All four touch event impls
// below are patched identically; leaving any one on `slot()` would key that
// event class differently from the rest.
//
// No offline test covers this: the libinput backend cannot be instantiated
// without a real device, which is the whole point of the ladder's offline
// discipline. Rung E-5 owns the live proof, on a machine with two touch devices.
