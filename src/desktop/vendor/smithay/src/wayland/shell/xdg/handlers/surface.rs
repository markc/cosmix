        ...
        match request {
            xdg_surface::Request::Destroy => {
                ...
                if data.has_active_role.load(Ordering::Acquire) {
                    // xdg_shell names this exactly: `defunct_role_object`, on
                    // the destroyed `xdg_surface`. It used to be reported as
                    // `xdg_wm_base.role`, which is the code for a *role*
                    // conflict and names the wrong object.
            ...
            xdg_surface::Request::GetToplevel { id } => {
                ...
                // A second role object on *this* wrapper. The role object's
                // `destroy` clears the flag, so re-taking the role after
                // destroying the previous one stays legal.
                if data.has_active_role.load(Ordering::Acquire) {
                    xdg_surface.post_error(
                        xdg_surface::Error::AlreadyConstructed,
                        "xdg_surface already has an active role object",
                    );
                    return;
                }

                // A second role object reached through a *different* wrapper.
                // Two role-less `xdg_surface`s may legally be created for one
                // `wl_surface` before either takes a role, and `give_role`
                // below cannot catch the second of them: `set_role` rejects
                // only a *different* role, so the same role passes straight
                // through. The live list is the only authority on this.
                if state
                    .xdg_shell_state()
                    .known_toplevels
                    .iter()
                    .any(|toplevel| toplevel.alive() && toplevel.wl_surface() == surface)
                {
                    shell.post_error(
                        xdg_wm_base::Error::Role,
                        "wl_surface already has a live xdg_toplevel",
                    );
                    return;
                }
            ...
            xdg_surface::Request::GetPopup {
                ...
                // Both checks mirror `GetToplevel`; see the reasoning there.
                // They come first so that a duplicate is refused before any of
                // the positioner or parent state below is read.
                if data.has_active_role.load(Ordering::Acquire) {
                    xdg_surface.post_error(
                        xdg_surface::Error::AlreadyConstructed,
                        "xdg_surface already has an active role object",
                ...
                if state
                    ...
                    .known_popups
                    .iter()
                    .any(|popup| popup.alive() && popup.wl_surface() == &data.wl_surface)
                {
                    data.wm_base.post_error(
                        xdg_wm_base::Error::Role,
                        "wl_surface already has a live xdg_popup",
                    );
                    return;
                }
            ...
            xdg_surface::Request::SetWindowGeometry { x, y, width, height } => {
                ...
                // `has_active_role`, not `role.is_some()`. The role lives on
                // the `wl_surface` and is permanent, so a wrapper that never
                // constructed a role object still sees `Some(..)` once any
                // *other* wrapper took the role -- and would then write into
                // the live role's shared `SurfaceCachedState` below.
                if !data.has_active_role.load(Ordering::Acquire) {
                    xdg_surface.post_error(
            ...
            xdg_surface::Request::AckConfigure { serial } => {
                ...
                // `has_active_role`, not the surface's permanent role -- see
                // `SetWindowGeometry` above. A role-less wrapper reaching here
                // would consume the *live* role object's configure serials.
                if !data.has_active_role.load(Ordering::Acquire) {
                    xdg_surface.post_error(
                        xdg_surface::Error::NotConstructed,
                        "xdg_surface must have a role.",
