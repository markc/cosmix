    ...
    compositor::with_states(surface, |states| {
            ...
            .get::<RefCell<Option<WpLinuxDrmSyncobjSurfaceV1>>>()
        {
            if let Some(syncobj_surface) = data.borrow().as_ref() {
                ...
                } else if new_buffer.is_some() {
                    // Reaching here means neither point is set: an earlier arm
                    // would have caught any commit where exactly one was. Stock
                    // 0.7.0 has no arm for this and returns silently, so a
                    // client that attaches a buffer and sets no points at all
                    // is never told — the one case the chain above misses.
                    //
                    // The protocol is normative that this is an error, and it
                    // is an "if and only if": "Clients must set both acquire
                    // and release points if and only if a non-null buffer is
                    // attached in the same surface commit."
                    // (linux-drm-syncobj-v1.xml:145-147). The condition is
                    // therefore on a *non-null* attachment — `new_buffer` is
                    // Some only for BufferAssignment::NewBuffer, so a null
                    // attach, or a commit that attaches nothing at all, stays
                    // legal and silent here.
                    //
                    // Which error code is genuinely under-determined, and the
                    // per-request rules are what make it so rather than leave
                    // it unsaid. set_acquire_point: "If at surface commit time
                    // there is a pending buffer attached but no pending acquire
                    // timeline point set, the no_acquire_point protocol error
                    // is raised" (xml:201-203). set_release_point says the same
                    // for no_release_point (xml:252-254). When neither point is
                    // set both rules fire at once, and nothing in the spec
                    // orders them. Emitting no_acquire_point (4) is
                    // this project's choice, made because the acquire point is
                    // what gates sampling and so is the first thing a
                    // conforming client is missing. Upstream may reasonably
                    // pick 5; a future bump that does is not a regression here,
                    // but the reproducer asserting 4 would need updating with
                    // it.
                    syncobj_surface.post_error(
                        wp_linux_drm_syncobj_surface_v1::Error::NoAcquirePoint as u32,
                        "buffer attached without acquire or release point".to_string(),
                    );
                }
            }
        }
...
fn signal_release_point(state: &DrmSyncobjCachedState) {
...
fn destruction_hook<D: DrmSyncobjHandler>(_data: &mut D, surface: &WlSurface) {
    compositor::with_states(surface, |states| {
        let mut cached = states.cached_state.get::<DrmSyncobjCachedState>();
        // Defect 2 of the two this vendoring exists to fix. Stock 0.7.0 signals
        // `pending` and `current` only, which is every state a surface has when
        // its commits apply immediately. A synchronized subsurface's commits do
        // not: they queue in the serial-keyed cache between the two until the
        // parent commit applies them. Destroy such a surface with commits still
        // queued and each cached release point is never signalled — the client
        // waits on that timeline point forever, and it is exactly the buffer
        // release the destroyed surface owed it.
        //
        // Every cached state is signalled, with no dedupe. Duplicates can only
        // arise from a client reusing a point value across queued commits, and
        // the protocol already tells clients that release points may be
        // signalled out of order, so a second signal on an already-signalled
        // point is not something a conforming client can be surprised by.
        // Skipping one, by contrast, is the defect.
        //
        // The order — pending, then cached oldest-first, then current — is the
        // smallest change from what was here, not a protocol requirement:
        // signalling is not ordered against itself.
        signal_release_point(cached.pending());
        for state in cached.cached() {
            signal_release_point(state);
        }
        signal_release_point(cached.current());
    });
}
    ...
    fn request(
        ...
        match request {
            wp_linux_drm_syncobj_manager_v1::Request::GetSurface { id, surface } => {
                ...
                // Defect 3 of the three this vendoring exists to fix, and the
                // one the fix to `destruction_hook`'s body could not reach: the
                // hook has to still be installed to run at all.
                //
                // Stock 0.7.0 tied the destruction hook to the lifetime of the
                // extension object, adding it here and removing it in the
                // extension's destroy handler. A client that destroys the
                // wp_linux_drm_syncobj_surface_v1 and *then* the wl_surface —
                // an ordinary teardown order, and one the protocol nowhere
                // forbids — therefore left nothing behind to signal the release
                // points still sitting in the commit cache, and every one of
                // them was stranded.
                //
                // The hook is now per-`wl_surface`, installed the first time
                // any syncobj extension is created for it and never removed:
                // it is the surface's cleanup, not the extension's. The marker
                // below is what makes it exactly once —
                // `insert_if_missing` returns true only for the insertion that
                // actually happened, so an extension destroyed and recreated on
                // the same surface reuses the existing hook instead of stacking
                // another copy on it.
                //
                // Leaving the hook on a surface whose extension is gone is not
                // a tolerated cost, it is the point. "Any timeline point set
                // by this object before the last commit will not be affected"
                // (linux-drm-syncobj-v1.xml:161-162) — committed points
                // outlive the extension, so the surface that has no live
                // extension is exactly the surface whose release points still
                // need signalling when it dies. That is the case stock 0.7.0
                // dropped. The residual cost is a hook that captures nothing
                // and, on a surface that really has no points left, finds every
                // slot empty.
                let destruction_hook_is_new = with_states(&surface, |states| {
                    states
                        .data_map
                        .insert_if_missing(|| SyncobjDestructionHookInstalled)
                });
                if destruction_hook_is_new {
                ...
                // Defect 4. This was `insert_if_missing(|| RefCell::new(Some(..)))`,
                // which is correct exactly once per surface. The extension's
                // destroy handler sets the stored value to None and leaves the
                // entry in place, so on any surface that has had an extension
                // before, the entry is present-but-None, `insert_if_missing`
                // declines, and the recreated object is dropped on the floor:
                // `commit_hook` finds nothing to post errors on and silently
                // stops validating that surface, while the stale entry also
                // makes the `already_exists` guard above answer false forever.
                //
                // The slot is created if absent and then written
                // unconditionally, so a recreated extension is recorded exactly
                // like a fresh one.
                with_states(&surface, |states| {
                    states
                        .data_map
                        .insert_if_missing(|| RefCell::new(None::<WpLinuxDrmSyncobjSurfaceV1>));
                    *states
                        .data_map
                        .get::<RefCell<Option<WpLinuxDrmSyncobjSurfaceV1>>>()
                        .unwrap()
                        .borrow_mut() = Some(syncobj_surface);
                });
            }
...
/// Marker left in a `wl_surface`'s user data the first time a syncobj extension
/// is created for it, recording that the destruction hook is installed.
///
/// The hook belongs to the surface and outlives any individual extension
/// object, so there is no id to keep — the only question ever asked is whether
/// it is already there.
struct SyncobjDestructionHookInstalled;
        ...
        match request {
            wp_linux_drm_syncobj_surface_v1::Request::Destroy => {
                if let Ok(surface) = data.surface.upgrade() {
                    ...
                    // Only the pre-commit hook goes. The destruction hook is
                    // the surface's, not this object's — removing it here is
                    // what stranded the cached release points of a client that
                    // destroyed the extension before the surface.
                    with_states(&surface, |states| {
                        *states
                            .data_map
                            .get::<RefCell<Option<WpLinuxDrmSyncobjSurfaceV1>>>()
