...
impl<T> CachedState<T> {
    ...
    /// Access the committed-but-not-yet-applied states for `T`, oldest first.
    ///
    /// These are commits that have already happened but whose state is not yet
    /// `current`, because the transaction owning them has not completed — the
    /// commit of a synchronized subsurface, waiting on its parent. Nothing
    /// outside this module could reach them, which is how a protocol handler
    /// tearing down a destroyed surface came to see only `pending` and
    /// `current` and miss everything in between.
    ///
    /// Crate-private on purpose. It hands out `&mut T` to state the transaction
    /// machinery owns, so it is a lifecycle tool for the protocol handlers in
    /// this crate, not part of the compositor-facing API; and the commit ids
    /// stay hidden, so the `VecDeque<(Serial, T)>` representation is still free
    /// to change.
    pub(crate) fn cached(&mut self) -> impl Iterator<Item = &mut T> {
        self.cache.iter_mut().map(|(_, state)| state)
    }
}
