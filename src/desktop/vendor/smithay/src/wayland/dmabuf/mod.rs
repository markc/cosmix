...
impl DmabufParamsData {
    ...
    fn create_dmabuf(
        ...
        // The extent check further down exempts planes above index zero, because
        // a subsampled plane is legitimately smaller than `stride * height`.
        // That exemption is only sound if the index it reads is the index the
        // plane ends up with — and the drain below relabels planes by their
        // position in this vector, which is client *arrival* order. Without this
        // guard a client can add its only plane as index 1, be exempted here as
        // a subsampled plane, and still be handed to the compositor as plane 0:
        // an undersized import that no later check can recover, since the
        // client's index is gone by then. Requiring the set to be exactly
        // `0..n` makes the two indices agree, and incidentally stops
        // out-of-order `add` requests from permuting the planes.
        planes.sort_unstable_by_key(|plane| plane.plane_idx);
        if planes
            .iter()
            .enumerate()
            .any(|(position, plane)| plane.plane_idx as usize != position)
        {
            params.post_error(
                zwp_linux_buffer_params_v1::Error::Incomplete,
                "Plane indices must be consecutive starting at zero",
            );
        ...
        // Carry the client's own index rather than the position in this vector.
        // The guard above makes the two equal, so no test can tell these apart
        // and this line is written this way precisely so that it does not have
        // to be tested: the guard is a check someone may later relax, whereas
        // an index that is never derived from a position cannot drift from the
        // one the client declared. `DmabufBuilder::build` sorts by the index it
        // is given, so this is also what puts the planes in canonical order.
        for plane in planes.drain(..) {
            let plane_idx = plane.plane_idx;
            let offset = plane.offset;
            let stride = plane.stride;
            buf.add_plane(plane.into(), plane_idx, offset, stride);
        }
