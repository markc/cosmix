...
// landing inside an upstream item. Fully qualified rather than adding to the
// `use` list above, for the same reason. See vendor/README.md.
impl DeviceFd {
    /// Weak reference to the underlying open file.
    ///
    /// A cleanup path that must release a kernel resource living in this file's
    /// handle table needs to know whether the file is still open without
    /// keeping it open itself. Upgrading the result succeeds exactly while some
    /// other owner holds the file.
    pub(crate) fn downgrade(&self) -> std::sync::Weak<OwnedFd> {
