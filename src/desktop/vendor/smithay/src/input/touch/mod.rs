...
impl<D: SeatHandler + 'static> TouchInternal<D> {
    ...
    // COSMIX PATCH: cancel is unconditional; it must not carry `frame`'s guard.
    //
    // `frame`'s `current >= pending` test suppresses a duplicate frame for a slot
    // that has had no event since its last one. Applying the same test to `cancel`
    // is wrong, because after any ordinary `down` + `frame` the test is *true* for
    // every live slot — so the cancel was skipped exactly when there was something
    // to cancel, sending no `wl_touch.cancel` and leaving the focus in place. That
    // contradicts this method's own contract ("remove all current focus targets")
    // and the protocol, where `wl_touch.cancel` ends the whole touch session for
    // the client rather than one slot and is not followed by a frame.
    //
    // Every slot is drained, so a `frame_pending` focus parked by `up` is dropped
    // rather than framed. One serial is used for all of them so that
    // `for_each_focused_touch`'s per-instance `last_seq` filter still collapses
    // multiple slots on one client to a single `wl_touch.cancel`.
    fn cancel(&mut self, data: &mut D, seat: &Seat<D>, seq: Serial) {
        for (_, state) in self.focus.drain() {
            if let Some((focus, _)) = state.focus {
