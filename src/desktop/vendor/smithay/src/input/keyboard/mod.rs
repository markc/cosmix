...
impl<D: SeatHandler + 'static> KeyboardInnerHandle<'_, D> {
    ...
    pub fn set_focus(
        ...
        if let Some(focus) = focus {
            ...
            // COSMIX FIX 7: upstream 0.7.0 omits this call, so focus-to-`None`
            // is the one focus *change* the handler never hears about, although
            // this method's own documentation says a `None` focus is a change
            // like any other. Both focus-installing arms above call it — the
            // focus-unchanged arm correctly calls nothing, and this, the sole
            // focus-clearing arm, returned straight after `leave`. (`None` ->
            // `None` matches no arm at all and is rightly silent.) The
            // compositor is therefore told nothing
            // when focus goes away, and any bookkeeping it keys on focus — for
            // us `xdg_toplevel::State::Activated`, the data-device focus, and
            // the serial `xdg_popup.grab` validates against — keeps its
            // previous value while the client has already been sent
            // `wl_keyboard.leave`.
            //
            // Placed after `leave` so the handler observes the same order a
            // client does. It cannot fire twice or fire spuriously: `take()`
            // has already yielded `Some`, so reaching here means focus really
            // was set and is now cleared.
            data.focus_changed(self.seat, None);
        }
    }
}
