//! `wl-iced-demo`: the raw grid under iced chrome (tabs, menu bar as xdg_popups, search field).
//!
...
use cosmix_wl_iced_demo::{iced::IcedDemo, raw::WAKE_EXIT};
...
fn main() {
    ...
    match cosmix_wl_app::run(runner) {
        Ok(stats) => {
            ...
            eprintln!(
                "wl-iced-demo: exit frames_committed={} buffer_allocations={} startup_ms={startup}",
                stats.frames_committed, stats.buffer_allocations
            );
        }
        Err(e) => {
            eprintln!("wl-iced-demo: {e}");
            std::process::exit(1);
        }
    }
}
