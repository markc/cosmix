...
fn verify(app: &mut App, sf: bool, suffix: &str) {
    let samples: Vec<_> = app
        ...
        .iter(app.world())
        .filter(|(text, _, _)| {
            matches!(text.0.as_str(), "Menu title" | "Menu item" | "Context item")
        })
    ...
    assert_eq!(
        samples.len(),
        3,
        "exercise actual bar, dropdown and context spawners"
    );
...
#[test]
fn menus_shape_installed_sf_faces_and_render() {
...
#[test]
fn menus_without_sf_use_explicit_free_family() {
