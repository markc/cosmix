...
```mix
print(run_argv(["env", "COSMIX_E2E_MIX_BIN=" .. env("COSMIX") .. "/src/target/release/mix", "cargo", "test", "--manifest-path", "desktop/Cargo.toml", "-p", "cosmix-term", "p0i_01_production_term_spawn_enrols_real_mix_and_exit_revokes", "--", "--nocapture"], {cwd: env("COSMIX") .. "/src"}))
```

The test shells out to `git status --porcelain` and compares live HEAD to the
binary's embedded full SHA, rejecting dirty or stale builds. There is no
installed-binary fallback. Without the variable it prints **SKIPPED** directly
to stderr, even under libtest capture; that ordinary desktop pass supplies no
production enrolment evidence.
