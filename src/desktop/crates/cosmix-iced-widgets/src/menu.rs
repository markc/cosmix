...
impl State {
    ...
    fn key<Message: Clone>(
        ...
        match key {
            ...
            Named::Enter | Named::Space => return self.activate(items, bar),
            Named::ArrowRight => {
...
fn title_width<Message>(item: &Item<Message>, style: MenuStyle) -> f32 {
    item.label.chars().count() as f32 * style.text_size * 0.7 + style.padding * 2.0
}

fn bar_rects<Message>(
    items: &[Item<Message>],
    bounds: Rectangle,
    style: MenuStyle,
) -> Vec<Rectangle> {
    ...
    items
        .iter()
        .map(|item| {
            let width = title_width(item, style);
            let rect = Rectangle {
                x,
                y: bounds.y,
                width,
    ...
    for Menu<'_, Message, Theme, Renderer>
{
    ...
    fn draw(
        ...
        for (index, (item, rect)) in self
            .items
            .iter()
            .zip(bar_rects(&self.items, layout.bounds(), self.style))
            .enumerate()
        {
            let selected =
                (state.open && state.root == index) || (item.selectable() && cursor.is_over(rect));
            if selected {
        ...
        match event {
            Event::Mouse(mouse::Event::ButtonPressed(button)) => {
                ...
                if state.focused && bar && *button == mouse::Button::Left {
                    if let Some(index) = bar_rects(&self.items, layout.bounds(), self.style)
                        .iter()
    ...
    fn mouse_interaction(
        ...
        } else if bar_rects(&self.items, layout.bounds(), self.style)
            .iter()
            .zip(&self.items)
    ...
    for Popup<'_, Message>
{
    fn layout(&mut self, _renderer: &Renderer, bounds: Size) -> layout::Node {
        ...
        let mut position = if self.bar {
            bar_rects(self.items, self.anchor, self.style)
                .get(self.state.root)
        ...
        for depth in 0..self.state.path.len() {
            ...
            let width = items
                .iter()
                .map(|item| {
                    title_width(item, self.style)
                        + item.accelerator.chars().count() as f32 * self.style.text_size * 0.7
    ...
    fn update(
        ...
        match event {
            ...
            Event::Keyboard(_) => shell.capture_event(),
            ...
            | Event::Mouse(mouse::Event::ButtonPressed(mouse::Button::Left)) => {
                ...
                if let Some((depth, index)) = self.hit(layout, cursor) {
                    ...
                    && let Some(index) = bar_rects(self.items, self.anchor, self.style)
                        .iter()
                        .position(|rect| cursor.is_over(*rect))
                {
                    if self.items[index].selectable() && self.state.root != index {
