...
fn duplicate_ids(text: &str) -> HashMap<String, usize> { let mut counts = HashMap::new(); for line in text.lines() { let trimmed = line.trim_start(); if trimmed.len() == line.len() { if let Some((id, _)) = line.split_once(':') { if id.chars().all(|c| c.is_ascii_alphanumeric() || c == '_' || c == '-') { *counts.entry(id.to_string()).or_default() += 1; } } } } counts }
