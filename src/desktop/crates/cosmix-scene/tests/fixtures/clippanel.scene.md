...
Handler ports (`on_click`, `on_change`, `on_submit`) name the citizen's
handlers: the host sends `clippanel-citizen <name>` with args
`{scene, node, kind, value?, item?}`. Rev 2 (2026-09-16 late): the ports
were missing from rev 1 — a P3 review finding.
