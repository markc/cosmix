...
/// Build a "User" submenu from script TOML files for the given service.
///
...
pub fn user_menu(service_name: &str) -> MenuItem {
    ...
    for (id, def) in &scripts {
        let action_id = format!("script:{id}");
        if let Some(ref shortcut_str) = def.script.shortcut {
            if let Some(shortcut) = parse_shortcut(shortcut_str) {
                items.push(action_shortcut(&action_id, &def.script.name, shortcut));
            } else {
                items.push(action(&action_id, &def.script.name));
            }
        } else {
            items.push(action(&action_id, &def.script.name));
        }
...
pub async fn handle_script_action(
    ...
    let scripts = discover_scripts(service_name);
    let (_, def) = scripts.iter().find(|(id, _)| id == script_id)?;

    let mut ctx = ScriptContext::new(service_name, vars.clone());
    let result = executor::execute(def, &mut ctx, hub).await;
