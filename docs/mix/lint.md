...
| `MIX-D3016` | a **script with no `-- version: X.Y.Z` header** in its first 32 lines (0.95.0), so [`mix SCRIPT --version`](invocation.md#--version-for-scripts) and `script_version()` can only say `unversioned` — every binary and every Mix script answers `--version` (Mark, 2026-09-25). Emitted by the lint driver, only for files **run as scripts**, by a deliberately simple rule: line 1 is a `#!` shebang, or the path *as given* has a `bin` or `_bin` directory component (so a `lib/` module loaded by `require` is never reported, and `mix lint deploy.mix` from inside `_bin/` is not either — lint from the hub root). A `-- version:` line whose value is not `X.Y.Z` is reported on its own line, since the runtime treats it as absent. A note by default; `--require-version` makes it a warning, code unchanged |
