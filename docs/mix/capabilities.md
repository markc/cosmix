...
`include` and `source` (which read a file off disk) are likewise gated as `FsRead` at the evaluator, not left ungated. The bare `send`/`emit` broker forms are gated `Bus` at their exec sites (since 0.89.0 — before that they bypassed the policy entirely). The principle: **every path that reaches host authority is gated**, whether it is a named builtin or a piece of syntax. See [shell dispatch](shell-mode.md) for what `sh`/`$()`/pipes do and [Bus messaging](bus.md) for `send`/`emit`/`on`. For the one-expression embedding entry point that bakes this deny-set into a static pre-execution walk, see [expression evaluation mode](expr-eval.md).
