...
| **Process** | Controls other processes / process lifecycle. | `run` `run_rc` `run_stream` `spawn` `kill` `process_alive` `audio_watch` `audio_unwatch` `audio_state` `chdir` `exit` `panic` |
| **Env** | Reads host/environment info or stdin. | `env` `cwd` `hostname` `pid` `platform` `which` `net_watch` `net_unwatch` `net_state` `readline` `read_stdin` |
...
- The **`Bus` seam** (`BusCallHandler`, installed via `set_bus_call_handler`) is a **delegated** Bus control-plane channel — deliberately **not** the raw broker (`BusHandler`, the `send`/`emit` keywords, which are gated `Bus` like `bus_call` since 0.89.0, so a policy that withholds `Bus` denies the bare forms too; a sandboxed handler still only *reaches* the mesh through the seam). The embedder bounds **which verbs** are reachable (a per-route exact-verb allowlist) and injects the **delegation envelope** — the authenticated actor, vhost, route — from *trusted request state*. The script calls `bus_call(verb, args)` and names **no host, peer, or actor**: it cannot forge identity or reach an unlisted verb. This is how a sandboxed webd handler calls a maild verb under the requesting user's delegated identity without holding broker authority.
...
`include` and `source` (which read a file off disk) are likewise gated as `FsRead` at the evaluator, not left ungated. The bare `send`/`emit` broker forms are gated `Bus` at their exec sites, and so are the **implicit sends an `address "target" … end` block's body lines desugar to** — a third dispatch site, not a path through `send` — and the **six evaluator-reserved Bus builtins** (`port_exists`, `bus_reconnect`, `noded_register`, `subscribe`, `unsubscribe`, `reply`), which sit outside the BUILTINS table and so failed class resolution open to `Pure` (all since 0.89.0 — before that all of these bypassed the policy entirely). The principle: **every path that reaches host authority is gated**, whether it is a named builtin or a piece of syntax. See [shell dispatch](shell-mode.md) for what `sh`/`$()`/pipes do and [Bus messaging](bus.md) for `send`/`emit`/`on`. For the one-expression embedding entry point that bakes this deny-set into a static pre-execution walk, see [expression evaluation mode](expr-eval.md).
