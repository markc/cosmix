...
Every Mix builtin grouped by category, generated from `mix builtins` (mix 0.82.0). See the linked topical page for prose and examples; `mix what NAME` prints a one-line description of any single builtin (or keyword), and `mix help` prints the compact names-only summary of the same ten categories.
