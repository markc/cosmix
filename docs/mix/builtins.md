...
Every Mix builtin grouped by category, generated from `mix builtins` (mix 0.90.0). See the linked topical page for prose and examples; `mix what NAME` prints a one-line description of any single builtin (or keyword), and `mix help` prints the compact names-only summary of the same ten categories.
...
## io  — see [io](io.md)
  ...
  fs_watch        Linux inotify watch, opaque evaluator-owned handle; options {recursive,events}; emits fs.changed {watch,changes,overflow} in $event.args
  fs_unwatch      Cancel a filesystem handle and pending changes; returns nil
  fs_wait         Wait for a filesystem batch without polling; refused in serve and expression modes
...
## system  — see [system](system.md)
  ...
  spawn           Start a child and return its PID. String form uses a shell; argv form runs directly. Options {detach,die_with_parent,exit_event,tag,cwd,env,clear_env,stdout,stderr}. exit_event:true registers the evaluator as sole reaper and emits proc.exited {pid,tag,exit_code,signal}; tag is a string. Managed children retire with their evaluator generation after handler drain. Without exit_event, existing detached/owned/plain spawn semantics apply. See system manual for lifetime rules.
  net_watch       Linux rtnetlink link/address subscription, opaque evaluator-owned handle; options {events}; emits net.changed {watch,changes,overflow,closed?} in $event.args; no polling
  net_unwatch     Cancel a net_watch handle and its pending changes; returns nil
  net_state       Snapshot {links, addresses} from an rtnetlink dump; re-read after any net.changed batch
  audio_watch     PipeWire change notices through one managed `pactl subscribe` child; options {facilities,runtime_dir}; emits audio.changed {watch,changes,overflow,closed?}
  audio_unwatch   Cancel an audio_watch handle: kills and reaps its child; returns nil
  audio_state     Default sink {ok,volume,level,muted,reason?} via one wpctl call with a 2 s deadline; no sink is ok:false, not an error
