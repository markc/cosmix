...
credential verification. Identity startup is asynchronous. An unavailable
profile or failed grant leaves ordinary panes usable, including Ctrl+Shift+T.
...
An idle expired bundle is held, not continuously replaced: opening a later
pane advances the model ID and requests another bundle. Consequently an early
startup pane or a pane opened after an idle grant expiry can be unbound even
with a healthy broker. Unbound running panes are not retroactively bootstrapped.
...
re-arms recovery. Attempts are charged before mutation, including uncertain
...
Each external event still costs names, so this is a bound on autonomous churn,
not a promise that arbitrary restarts or reconnects cannot exhaust the broker.
...
at most eight seconds and never joins an unfinished worker. Failed child closes
