...
or leave holds behind, and cannot keep screen space reserved (shell design §7).
Comp identifies Quoin by the Wayland client of its layers,
...
input; nothing of any other client is touched. While it is stopped its layers
reserve no space: their exclusive zones count as zero, so maximised
windows grow into what a docked panel held (the panel itself stays
drawn), and the reservation returns when Quoin is heard from again (an
acknowledged configure, a report, or a new connection). Stopped-ness is only
ever learned from those checks, so a Quoin whose every edge is docked or
pinned — nothing showing that comp conceals, no menu — is not probed and keeps
its reservation. A conceal Quoin applies (the
