...
- **Training log.** Each single-message training event logs one line under the tracing target `maild::bayesian::train`. This covers an IMAP move across Junk (drained by the retrain worker), a JMAP move, and `maild.bayesian.train`. `maild.bayesian.rebuild` does not log per message. It trains a shadow corpus in bulk and reports its counts in the job body instead. The line carries `account`, `direction` (`spam` / `ham`), `stamp` (the item id), `message_id`, `result` (`applied`, `already_labeled`, or `error`) and `via` (`imap`, `jmap`, `bus`). `maild.bayesian.untrain` logs a different line under the same target, with `account`, `direction=untrain`, `stamp`, `removed` (`spam`, `ham`, or `none` when the message carried no label) and `via=bus`; it has no `message_id` or `result`. An inline label (a JMAP move, `train` or `untrain`) takes a microsecond event instant when it starts. Once its write succeeds, it cancels the IMAP moves of the same message that were queued before that instant and logs `result=superseded` with the count. A move queued after the instant is a newer user action; it is kept and drains after the inline label. If the inline write fails, nothing is cancelled. Queued rows carry the instant in `mail_retrain_outbox.created_us` (MDS schema v10); rows from before v10 count as older. The drain skips a row cancelled after it was claimed, so an older queued event is never replayed over a newer label. `maild.bayesian.stats` reports `labelled_spam`, `labelled_ham` and `last_trained_at`, so you can confirm that a correction was learned without comparing corpus totals.
