...
|---|---|---|
| `term_list` | `{}` | Read-only JSON: `service` (the frontend it read, `bterm` or `term`), `instance` (that process's token; null from a frontend older than term-core 0.8.0) and arrays `tabs` and `panes`: ids, active flags, dimensions, child pids, tab titles and pane geometry. Sequential reads are not atomic; if the two reads came from different processes it errors instead. |
| `term_snapshot` | `{}` | Read-only active screen text, cols/rows, cursor, pid, byte counters and diagnostic timings. |
| `term_type` | `{"pane":3,"service":"bterm","instance":123,"text":"echo hello\n"}` | DIAGNOSTIC keyboard-encoder input; newline is Enter. `pane` or `tab` is required and never defaults to focus; `pane` is the safe selector, while `tab` means that tab's active pane at delivery and so still follows focus inside the tab. **Pass back the `service` and `instance` `term_list` returned**: pane ids are per-process counters from 1, so the call then goes to exactly that frontend with no fallback probe, and a different term process refuses (`INVALID_ARGUMENT`) instead of typing into its own pane 3. Omitted, both default to the last successful `term_list` in this MCP process; with no `term_list` yet the call is refused `INVALID_ARGUMENT` ("call term_list first"), and a terminal that restarted since is refused as "terminal restarted since term_list". The type path never probes or falls back to another frontend. Missing selectors, a zero id and an unknown service are errors without an ABP call. Not the authenticated input API. |
...
The MCP schemas use String for text/op, optional u64 for id/pane/tab/instance,
and optional String for dir/service. Invalid operations, missing required ids/directions, and
...
| `term.type` | `{"pane":3,"instance":123,"text":"..."}` (`pane` or `tab` required, `instance` optional) |
