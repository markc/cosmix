...
`AccountStats` reports message totals, approximate token totals, cold-start state, seed label, and model version. It also reports training counters: `labelled_spam` and `labelled_ham` count the messages that carry a training label, and `last_trained_at` is the Unix time of the newest label that still exists (`None` when no label remains). `last_trained_at` is not a monotonic history: untraining the newest label moves it back to the next-newest remaining one. The message totals include all seeded and imported mail. The label counters include only mail that has a label row. The seed is an exception: `open_account` copies it byte-for-byte and label rows record no origin, so any label rows the seed carried are counted as that account's own. A seed built without label rows adds nothing to the counters. The SQLite implementation reports the same unique-token count in both token-total fields.

`StorageBackend::peek_stats` (exposed as `DefaultClassifier::peek_stats`) is the read-only form for inspection. It creates no account directory or database, and it does not seed or promote one. `SqliteBackend` answers from a cached live connection when one is open. Otherwise it opens the first file that `open_account` would start from, read-only: `bayes.db`, then a legacy `db.sqlite`, then the seed. A read-only open of a live WAL database can still create its `-shm`/`-wal` sidecars in the account directory. The seed is a static file, so it is opened with `immutable=1`: nothing is written next to it, and a seed in a directory maild cannot write still works. When the answer comes from the seed, `seeded_from` names it. An account with none of these files reads as an empty cold-start corpus. The trait's default implementation calls `open_account`.
