...
| `cosmix-edit-core` | The text engine behind the [edit](edit.md) citizen and the [ced](ced.md) editor: gap-buffer text with a line index, byte-offset / line-col positions, all-or-nothing transactions, revision CAS and `base_rev` transforms (`ot`, including `txn_sequence` shared by server and client), per-origin undo lanes, anchors, selections and search, plus `view`: grapheme-correct navigation, word motion and cell measurement (wide characters, tabs, ambiguous width as a setting) with 4 KiB seek checkpoints. Pure Rust, no Bus and no async runtime. Vendors a subset of Microsoft [msedit](https://github.com/microsoft/edit) (MIT, Copyright (c) Microsoft Corporation): the gap buffer, SIMD line scanner, Unicode tables, measurement and word navigation, with provenance and licence in `src/vendor/msedit/`. |
