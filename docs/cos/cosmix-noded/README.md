...
Connections may register one service name with `noded.register`. Registered names are unique. Re-registering the same connection under a different name removes its old binding, while a collision with another live connection is rejected. The broker's own name, `noded`, and the `noded-` namespace are reserved and refused as `reserved_name`, so a locally routed `from: noded` always means this node's broker. A reply relayed from a mesh peer carries the remote's headers untouched (plus `broker_origin: mesh`), so a consumer that trusts `from: noded` should also require `broker_origin` absent or `local`.
