---
title: Cosmix Design System — the Resolved Design Graph
chapter: 19
version: 0.7.0
status: draft
date: 2026-08-09
...
# Chapter 19 — The Cosmix Design System

> Keep the four things CSS was smuggling in — named tokens, variants as
> data, live hot-swap, one mutable source — and delete the matching
> engine that came with them. Data chooses values; Rust owns structure;
> a compiler stands between the two and refuses to pass along anything
> it cannot prove.

## §0 Scope, provenance, and governing law

This chapter specifies the desktop-wide styling architecture for CosMix:
the `.mix` design source, the design compiler and its `ResolvedDesign`
artifact, the state model, family resolvers and writer ownership,
overrides, derivations, adapters, and the introspection surface. It
covers CTK chrome **and** app-defined widget families (decision 8) — the
whole desktop, one contract.

- **Decision record:** all architectural choices herein were accepted by
  Mark 2026-08-09
  (`_decisions/2026-08-09-resolved-design-graph-adopted.md`, ten
  decisions). Design rationale and evidence:
  `_doc/2026-08-09-cosmix-desktop-design-system.md` (rev 5). This
  chapter states the contracts; it does not re-argue them.
- **Governing law:** the three substrate criteria (legible / modifiable /
  reconstructible by agents) and the standing rules: `.mix` strict-data
  is the styling source of truth (2026-07-22 palette ADR, unamended);
  CTK follows shadcn component semantics; backends are event-driven,
  never polled.
- **Evidence base:** ctk 0.48.0 (`$COSMIX/src/desktop/ctk/`) — the shipped
  lift model (`style.rs`), `RadiusScale`, the live theme lane, the
  button family's change-tick-honesty discipline; the flair evaluation's
  writer inventory and audit design
  (`_plan/2026-08-09-bevy-flair-adoption-evaluation.md`).
- **Out of scope:** widget behaviour (headless logic, focus routing,
  accessibility semantics), the compositor (Ch16), the ABP display lane
  (Ch05), animation/transition tokens and a `SurfaceLevel`-style ambient
  context axis (both deferred by decision; §15).
- **Conformance language:** MUST / MUST NOT / SHOULD / MAY per RFC 2119
  usage. Every MUST in this chapter is backed by a named test,
  compile-time rejection, or — where mechanically untestable — an
  explicit review obligation; all are listed in §15.

Terminology: a **family** is a widget kind with declared variant/size
axes and parts (button, meter, browser-row…). A **part** is a named
member entity of a multi-entity widget (`ButtonRoot`, `ButtonLabel`). A
**cell** is one `variant × size × state` coordinate of a family's
resolved table. A **pair** is an atomic surface+foreground colour unit.
A **resolver** is the one Rust system that writes a family's
